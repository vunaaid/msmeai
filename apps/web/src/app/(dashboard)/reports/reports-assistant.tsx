...
// src/app/(dashboard)/reports/reports-assistant.tsx
// Trợ lý AI Báo cáo & Phân tích — cùng rule trợ lý, skill Tài chính/Quản trị.

import { useState, useEffect, useRef, useCallback } from "react";
import { Bot, Send, Loader2, Sparkles, Save, Activity, TrendingUp, Lightbulb } from "lucide-react";
...
const QUICK = [
  { key: "health", label: "Sức khỏe tài chính", icon: Activity,
    prompt: "Đánh giá sức khỏe tài chính doanh nghiệp từ các chỉ số sau (doanh thu, chi phí, lợi nhuận, tiền mặt, công nợ AR/AP): [dán số liệu]. Nêu điểm mạnh/rủi ro." },
  { key: "trend", label: "Phân tích biến động", icon: TrendingUp,
    prompt: "Phân tích biến động doanh thu / chi phí / dòng tiền giữa các kỳ và nguyên nhân: [dán số liệu]." },
  { key: "advice", label: "Đề xuất cải thiện", icon: Lightbulb,
    prompt: "Dựa trên tình hình tài chính hiện tại, đề xuất 3-5 hành động ưu tiên để cải thiện dòng tiền & lợi nhuận cho doanh nghiệp SME." },
] as const;

export function ReportsAssistant({ roleName, roleLevel }: { roleName: string | null; roleLevel: string | null }) {
  ...
  useEffect(() => {
    apiFetch<AgentOpt[]>("/api/ai/agents/company")
      .then(({ data }) => {
        ...
        const match =
          find("tài chính") ?? find("cfo") ?? find("kế toán") ?? find("giám đốc") ??
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[calc(100vh-9rem)] min-h-[460px] overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-800 flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white truncate">Trợ lý Báo cáo</div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center gap-2 text-slate-500">
            ...
            <p className="text-xs max-w-xs">Trợ lý hỗ trợ <b>đánh giá sức khỏe tài chính, phân tích biến động, đề xuất cải thiện</b>.</p>
          </div>
        )}
        {messages.map((m) => (
      ...
      <div className="p-3 border-t border-slate-800">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            placeholder="Hỏi trợ lý báo cáo / phân tích..."
            rows={1}
