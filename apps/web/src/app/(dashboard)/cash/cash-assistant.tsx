...
// src/app/(dashboard)/cash/cash-assistant.tsx
// Trợ lý AI Ngân quỹ — cùng rule trợ lý, skill Tài chính/Kế toán.

import { useState, useEffect, useRef, useCallback } from "react";
import { Bot, Send, Loader2, Sparkles, Save, Tags, TrendingUp, Scale } from "lucide-react";
...
const QUICK = [
  { key: "classify", label: "Phân loại thu/chi", icon: Tags,
    prompt: "Phân loại các khoản thu/chi sau vào nhóm (lương, mua hàng, bán hàng, thuế, chi phí vận hành...): [dán danh sách giao dịch]." },
  { key: "forecast", label: "Dự báo dòng tiền", icon: TrendingUp,
    prompt: "Từ số dư quỹ hiện tại và các khoản thu/chi dự kiến sau, dự báo dòng tiền 3 tháng tới và cảnh báo nguy cơ âm quỹ: [dán dữ liệu]." },
  { key: "recon", label: "Đối chiếu quỹ", icon: Scale,
    prompt: "Hướng dẫn quy trình đối chiếu số dư sổ quỹ với sao kê ngân hàng / kiểm kê tiền mặt cuối kỳ." },
] as const;

export function CashAssistant({ roleName, roleLevel }: { roleName: string | null; roleLevel: string | null }) {
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[calc(100vh-9rem)] min-h-[460px] overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-800 flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white truncate">Trợ lý Ngân quỹ</div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center gap-2 text-slate-500">
            ...
            <p className="text-xs max-w-xs">Trợ lý hỗ trợ <b>phân loại thu/chi, dự báo dòng tiền, đối chiếu quỹ</b>.</p>
          </div>
        )}
        {messages.map((m) => (
      ...
      <div className="p-3 border-t border-slate-800">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            placeholder="Hỏi trợ lý ngân quỹ..."
            rows={1}
