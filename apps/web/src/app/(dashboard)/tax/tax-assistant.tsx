...
// src/app/(dashboard)/tax/tax-assistant.tsx
// Trợ lý AI Thuế — cùng rule trợ lý, skill Thuế/Kế toán.

import { useState, useEffect, useRef, useCallback } from "react";
import { Bot, Send, Loader2, Sparkles, Save, CalendarClock, Percent, BadgePercent } from "lucide-react";
...
const QUICK = [
  { key: "deadline", label: "Lịch nộp thuế", icon: CalendarClock,
    prompt: "Liệt kê thời hạn nộp tờ khai & nộp tiền các loại thuế (GTGT, TNDN tạm tính, TNCN, lệ phí môn bài) cho doanh nghiệp SME theo quy định VN hiện hành." },
  { key: "incentive", label: "Ưu đãi ngành nghề", icon: BadgePercent,
    prompt: "Doanh nghiệp ngành [nhập ngành nghề] có thể được hưởng ưu đãi thuế TNDN nào (thuế suất ưu đãi, miễn/giảm)? Căn cứ pháp lý." },
  { key: "calc", label: "Cách tính thuế", icon: Percent,
    prompt: "Giải thích cách tính thuế GTGT (khấu trừ vs trực tiếp) và TNDN cho doanh nghiệp, kèm ví dụ số liệu." },
] as const;

export function TaxAssistant({ roleName, roleLevel }: { roleName: string | null; roleLevel: string | null }) {
  ...
  useEffect(() => {
    apiFetch<AgentOpt[]>("/api/ai/agents/company")
      .then(({ data }) => {
        ...
        const match =
          find("thuế") ?? find("kế toán") ?? find("tài chính") ?? find("cfo") ??
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[calc(100vh-9rem)] min-h-[460px] overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-800 flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white truncate">Trợ lý Thuế</div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center gap-2 text-slate-500">
            ...
            <p className="text-xs max-w-xs">Trợ lý hỗ trợ <b>lịch nộp thuế, ưu đãi ngành nghề, cách tính GTGT/TNDN/TNCN</b>.</p>
          </div>
        )}
        {messages.map((m) => (
