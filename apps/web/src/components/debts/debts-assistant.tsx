...
// src/components/debts/debts-assistant.tsx
// Trợ lý AI Công nợ (AR/AP) — cùng rule trợ lý, skill Tài chính/Kế toán.

import { useState, useEffect, useRef, useCallback } from "react";
import { Bot, Send, Loader2, Sparkles, Save, AlarmClock, Mail, BarChart3 } from "lucide-react";
...
const QUICK = [
  { key: "remind", label: "Nhắc thu/trả nợ", icon: AlarmClock,
    prompt: "Soạn danh sách hành động nhắc thu/trả công nợ cho các khoản sắp đến hạn và quá hạn: [dán danh sách công nợ]." },
  { key: "email", label: "Email đòi nợ", icon: Mail,
    prompt: "Soạn email nhắc thanh toán lịch sự nhưng dứt khoát gửi khách hàng [tên] cho khoản công nợ [số tiền, số hóa đơn, hạn]." },
  { key: "aging", label: "Phân tích tuổi nợ", icon: BarChart3,
    prompt: "Phân tích tuổi nợ (aging) và rủi ro thu hồi từ dữ liệu sau, đề xuất ưu tiên xử lý: [dán dữ liệu công nợ]." },
] as const;

export function DebtsAssistant({ roleName, roleLevel }: { roleName: string | null; roleLevel: string | null }) {
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[calc(100vh-9rem)] min-h-[460px] overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-800 flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white truncate">Trợ lý Công nợ</div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center gap-2 text-slate-500">
            ...
            <p className="text-xs max-w-xs">Trợ lý hỗ trợ <b>nhắc thu/trả nợ, soạn email đòi nợ, phân tích tuổi nợ</b>.</p>
          </div>
        )}
        {messages.map((m) => (
      ...
      <div className="p-3 border-t border-slate-800">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            placeholder="Hỏi trợ lý công nợ..."
            rows={1}
